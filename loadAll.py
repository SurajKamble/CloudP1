...
for one in soup.find_all('a'):
    ...
    if x.startswith('javascript:rsrvInfo'):
        ...
        '''
        event_page = requests.get('http://calendar.uc.edu/wv3/wv3_servlet/urd/run/wv_event.Detail?id='+a)
...
def load_events(all_events):
    ...
    for event in all_events:
        ...
        if hall_name != "":
            ...
            c.execute("INSERT INTO events (event_name, location, event_time, description, hall_name) VALUES (%s, %s, %s, %s, %s)",
                      (event_name, location, event_time, description, hall_name))
            conn.commit()
